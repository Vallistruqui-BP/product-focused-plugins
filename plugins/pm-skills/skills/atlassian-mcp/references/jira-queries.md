...
#### Gotcha verificado: @mentions en comentarios necesitan ADF, no markdown

Con `mcp__atlassian__addCommentToJiraIssue`, la sintaxis clásica de mención `[~accountid:XXXX]` **no se interpreta** cuando `contentFormat` queda en markdown (el default): se postea como texto literal escapado, sin generar una mention real. Verificado en producción — hubo que reeditar varios comentarios ya publicados con este problema.

Forma correcta: `contentFormat: "adf"` + body como documento ADF con un nodo `mention` nativo por persona:
  ...
  "content": [
    {
      "type": "paragraph",
      "content": [
        { "type": "mention", "attrs": { "id": "<accountId>", "text": "@Nombre Visible" } },
        { "type": "text", "text": " resto del comentario..." }
      ]
    }
  ]
}
...
El `accountId` se obtiene antes con `mcp__atlassian__lookupJiraAccountId`. Un comentario ya publicado con la mención rota se corrige in place pasando `commentId` + `contentFormat: "adf"` + el body corregido, sin borrar y recrear. Detalle completo de esta regla, incluida la firma obligatoria de comentarios de Pickit, en la skill [`pickit-jira-comment-signature`](../../pickit-jira-comment-signature/SKILL.md).
